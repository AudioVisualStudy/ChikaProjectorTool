...
public class ChikaProjector : MonoBehaviour {
    ...
    public Camera targetCamera; // 実解像度の映像を描画するカメラ。未設定時は同じ GameObject の Camera。
    ...
    private void ConfigureDisplayQuads() {
        ...
        float halfHeight = designHeight * 0.5f;
        float topY = halfHeight;
        float bottomY = -halfHeight;

        ConfigureDisplayQuad(topQuad, topMaterial, new Vector3(0f, topY, 0f), new Vector3(halfWidth, designHeight, 1f));
        ConfigureDisplayQuad(bottomQuad, bottomMaterial, new Vector3(0f, bottomY, 0f), new Vector3(halfWidth, designHeight, 1f));
    }
